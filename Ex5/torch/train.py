...
def derivative(model, t):
    diff_x = torch.tensor(5e-5, dtype=torch.float32)
...
def train_fit():
    ...
    # x_train, x_test = x_perm.split([700, 300], dim=0)
    # Y_train, Y_test = Y_true_perm.split([700, 300], dim=0)
    ...
    model_fit = NetFit(save_path='fit/', save_name='netfit_1.pt')
    loss = nn.MSELoss()
...
def train_4():
    ...
    model4 = Net4(save_path='4/', save_name='net4_nar.pt')
    loss = nn.MSELoss()
...
def view_model():
    model_dict = torch.load('4/net4_nar.pt')
    print(model_dict)
...
def plot_sol():
    ...
    m1_1 = 1.33122
    m1_2 = 1.
    v1 = np.exp((1/2) * (0.7552 + np.sqrt(78.93402716)) * time_step)
    
    m2_1 = -0.59173
    m2_2 = 1.
    v2 = np.exp((1/2) * (0.7552 - np.sqrt(78.93402716)) * time_step)
